...
export default function NavBar() {
    return (
        <>
            <div className="relative bg-background font-display tracking-widest text-center text-primary-fg text-4xl mt-5 z-40"><a href="/" className="hover:scale-110 hover:tracking-[0.2em] transition-all motion-reduce:transition-none motion-reduce:hover:tracking-widest motion-reduce:hover:scale-100 hover:brightness-150 inline-block">Lexasaurus</a></div>
            <div className="sticky top-0 bg-background border-b border-midground w-full flex justify-between mt-1 lg:-mt-3">
                <div className="hidden md:flex">
                    {leftLinks.map((link) => (
                        <a key={link.label} href={link.href} className="bg-background px-5 py-3 transition hover:bg-midground">{link.label}</a>
                    ))}
                </div>
                <Menu>
                    ...
                    <MenuItems anchor="bottom start" className="relative md:hidden data-open mt-3 flex flex-col">
                        {leftLinks.map((link) => (
                            <MenuItem key={link.href} as="div">
                                <a href={link.href} className="block bg-background px-5 py-3 transition hover:bg-midground list-none">{link.label}</a>
                            </MenuItem>
                        ))}
                    </MenuItems>
                </Menu>
                ...
                <a href="/login" className="bg-background px-5 py-3 transition hover:bg-midground">login?</a>
            </div>
        </>
    );
}
